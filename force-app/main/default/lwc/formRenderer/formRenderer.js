...
export default class FormRenderer extends LightningElement {
  ...
  @api readOnly = false;
  /** When true, conversational layout does not emit finish (preview / non-persisting embeds). */
  ...
  get _recordContextJson() {
    if (this.recordContext) {
      if (typeof this.recordContext === "object")
        return JSON.stringify(this.recordContext);
  ...
  get selectedFormId() {
    return this._selectedFormId || this.formId;
  }
  set selectedFormId(val) {
    this._selectedFormId = val;
  }

  get submissionId() {
    return this._submissionId;
  }
  set submissionId(val) {
    this._submissionId = val;
  }

  get isSelectMode() {
    return this.mode === "select" && !this.isReadOnly;
  }
  get isRenderMode() {
  ...
  get showBackButton() {
    return this.showHeader && !this.formId && !this.isReadOnly;
  }
  ...
  get hasLanguages() {
    return (
      this.featureSettings.translations && this.availableLanguages.length > 1
    );
  }
  ...
  get progressSteps() {
    return this.visiblePages.map((p, idx) => ({
      ...p,
      translatedPageName:
        this.getTranslated("Page", p.pageId, "Name") || p.pageName,
      translatedPageDescription:
        this.getTranslated("Page", p.pageId, "C_Description__c") ||
        p.description,
      stepIndex: idx + 1,
      ...
      stepClass:
        idx === this.currentStep
          ...
          : idx < this.currentStep
            ? "step-pill step-completed"
            : "step-pill step-upcoming"
    }));
  }
  ...
  get currentPageDescription() {
    ...
    const page = this.currentPage;
    return (
      this.getTranslated("Page", page.pageId, "C_Description__c") ||
      page.description
    );
  }
  ...
  get isLastPage() {
    return this.currentStep >= this.visiblePages.length - 1;
  }
  ...
  get isClassic() {
    return this.rendererLayout?.toLowerCase() === "classic";
  }
  ...
  get isWizard() {
    return this.rendererLayout?.toLowerCase() === "wizard";
  }
  ...
  async loadForms() {
    ...
    try {
      ...
      this._notify(
        "Error",
        error?.body?.message || error?.message || "Failed to load forms",
        "error"
      );
    } finally {
  ...
  async _launchForm() {
    ...
    try {
      ...
      const result = await getFormStructureWithMeta({
      ...
      if (!this.isReadOnly) {
        ...
        const hasDraft =
          (subResult.isResume === true || subResult.isResume === "true") &&
          subResult.draftJson;
        if (hasDraft) {
      ...
      this._notify(
        "Error",
        error?.body?.message || error?.message || "Failed to load form",
        "error"
      );
    } finally {
      this.isLoading = false;
  ...
  async _loadReadOnlySubmission(subId) {
    ...
    try {
      ...
      if (
        subData.language &&
        subData.language !== this.formMeta.defaultLanguage
      ) {
      ...
      this._notify(
        "Error",
        error?.body?.message || error?.message || "Failed to load submission",
        "error"
      );
    } finally {
      this.isLoading = false;
  ...
  async _applyPrePopulation(hasDraft) {
    try {
      ...
      if (appliedCount > 0) {
        ...
        this._notify(
          "Pre-filled",
          `${appliedCount} field(s) were pre-populated from your records.`,
          "info"
        );
      }
    } catch (e) {
  ...
  _scheduleAutoSave() {
    ...
    this._clearAutoSave();
    // eslint-disable-next-line @lwc/lwc/no-async-operation -- debounced draft persistence
    ...
    try {
      ...
      if (!this.selectedLanguage) {
        this.selectedLanguage =
          this.formMeta?.defaultLanguage ||
          (langs && langs.length > 0 ? langs[0] : "");
      }
    } catch (error) {
  ...
  getTranslated(type, id, field) {
    if (!this.translationMap || Object.keys(this.translationMap).length === 0)
      return null;
  ...
  handleNav(event) {
    const direction =
      event?.detail?.direction || event?.currentTarget?.dataset?.direction;
  ...
  evaluateDependencies() {
    ...
    for (const dep of ungrouped) {
      const isTriggered =
        this.selectedResponses[dep.controllingResponse] === true;
      if (dep.action === "Show") {
    ...
    for (const key of Object.keys(grouped)) {
      ...
      const results = group.deps.map(
        (d) => this.selectedResponses[d.controllingResponse] === true
      );
    ...
    const visibleCount = (this.formStructure || []).filter(
      (p) => !hidden[p.pageId]
    ).length;
  ...
  async handleFinish() {
    if (this.isReadOnly) return;
    if (this.previewMode === true || this.previewMode === "true") return;
...
function crossFieldRuleFails(dep, ref, rule) {
  ...
  if (t === "Date") {
    const dv = trimAnswerString(
      dep.textValue != null ? dep.textValue : dep.value
    );
    const rv = trimAnswerString(
      ref.textValue != null ? ref.textValue : ref.value
    );
