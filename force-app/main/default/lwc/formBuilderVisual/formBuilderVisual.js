...
export default class FormBuilderVisual extends LightningElement {
  ...
  _modalTrapKey = null;
  _modalTrapPreviousFocus = null;
  _boundModalKeydown = null;
  ...
  _enrichPageTree(pages, pageCategoryParentKey) {
    const { targetPages, targetSections, targetQuestions } =
      this.dependencyTargetIds;
    const parentKey =
      pageCategoryParentKey === undefined || pageCategoryParentKey === null
        ? ""
        : pageCategoryParentKey;
    return pages.map((page, pageIdx) => {
      const sections = (
        this.formStructure?.sectionsByPage?.[page.Id] || []
      ).map((section, secIdx) => {
        const questions = (
          this.formStructure?.questionsBySection?.[section.Id] || []
        ).map((question, qIdx) => {
          const responses = (
            this.formStructure?.responsesByQuestion?.[question.Id] || []
          ).map((resp, rIdx) => ({
            ...resp,
        ...
        return {
          ...
          collapseIcon: this.collapsedIds[section.Id]
      ...
      return {
        ...
        collapseIcon: this.collapsedIds[page.Id]
          ? "utility:chevronright"
          : "utility:chevrondown"
      };
    });
  ...
  get canvasRows() {
    ...
    for (const dep of this.formStructure?.dependencies || []) {
      if (dep.C_Target_Category__c)
        targetCategories.add(dep.C_Target_Category__c);
    }
    const rows = categoriesRaw.map((category, catIdx) => {
      ...
      return {
        ...
        collapseIcon: this.collapsedIds[category.Id]
          ? "utility:chevronright"
          : "utility:chevrondown",
  ...
  filterCanvasRowsToRepositoryFocus(fullRows) {
    ...
    if (ft === "section") {
      ...
      targetPageId = targetSectionId
        ? this.findParentId("section", targetSectionId)
        : null;
    }
  ...
  get dependencyTargetIds() {
    ...
    for (const dep of this.formStructure?.dependencies || []) {
      ...
      if (dep.C_Target_Question__c)
        targetQuestions.add(dep.C_Target_Question__c);
    }
  ...
  get enrichedDependencies() {
    ...
    return this.dependencies.map((d) => {
      const respLabel =
        responseMap[d.C_Controlling_Response__c] || d.C_Controlling_Response__c;
      const qLabel = questionForResponse[d.C_Controlling_Response__c] || "";
      const targetName =
        d.C_Target_Type__c === "Category"
      ...
      return {
        ...d,
        controllingResponseLabel: qLabel
          ? `${qLabel} > ${respLabel}`
          : respLabel,
  ...
  get propertiesPanelTitle() {
    ...
    const typeLabel =
      this.selectedElementType.charAt(0).toUpperCase() +
      this.selectedElementType.slice(1);
    return `Properties - ${typeLabel}`;
  }

  get isFormProperties() {
    return this.selectedElementType === "form";
  }
  get isCategoryProperties() {
    return this.selectedElementType === "category";
  }
  get isPageProperties() {
    return this.selectedElementType === "page";
  }
  get isSectionProperties() {
    return this.selectedElementType === "section";
  }
  get isQuestionProperties() {
    return this.selectedElementType === "question";
  }
  get isSelectedQuestionTable() {
    return (
      this.isQuestionProperties &&
      this.selectedElement &&
      this.selectedElement.C_Type__c === "Table"
    );
  }
  get isSelectedQuestionFile() {
    return (
      this.isQuestionProperties &&
      this.selectedElement &&
      this.selectedElement.C_Type__c === "File"
    );
  }
  get isSelectedQuestionNumber() {
    return (
      this.isQuestionProperties &&
      this.selectedElement &&
      this.selectedElement.C_Type__c === "Number"
    );
  }
  get isSelectedQuestionLongText() {
    return (
      this.isQuestionProperties &&
      this.selectedElement &&
      this.selectedElement.C_Type__c === "Long Text"
    );
  }
  get isSelectedQuestionDate() {
    return (
      this.isQuestionProperties &&
      this.selectedElement &&
      this.selectedElement.C_Type__c === "Date"
    );
  }
  /** Show for any selected question so admins always find the panel; Date/Number get full editor inside. */
  get showCrossFieldRules() {
    return this.isQuestionProperties;
  }
  ...
  get isResponseProperties() {
    return this.selectedElementType === "response";
  }
  ...
  get parsedCrossFieldRules() {
    if (
      !this.selectedElement ||
      !this.selectedElement.C_Cross_Field_Rules_JSON__c
    )
      return [];
    try {
  ...
  get questionTypeOptions() {
    return QUESTION_TYPES.map((qt) => ({
      label: qt.label.replace(" Question", ""),
      value: qt.value
    }));
  }
  ...
  getCategoryCssClass(categoryId) {
    return this.selectedElement?.Id === categoryId &&
      this.selectedElementType === "category"
      ? "canvas-category is-selected"
      : "canvas-category";
  }

  getPageCssClass(pageId) {
    return this.selectedElement?.Id === pageId &&
      this.selectedElementType === "page"
      ? "canvas-page is-selected"
      : "canvas-page";
  }

  getSectionCssClass(sectionId) {
    return this.selectedElement?.Id === sectionId &&
  ...
  getQuestionCssClass(questionId) {
    return this.selectedElement?.Id === questionId &&
      this.selectedElementType === "question"
      ? "canvas-question is-selected"
      : "canvas-question";
  }

  getResponseCssClass(responseId) {
    return this.selectedElement?.Id === responseId &&
      this.selectedElementType === "response"
      ? "canvas-response is-selected"
      : "canvas-response";
  }

  get showScoring() {
    return this.featureSettings.scoring;
  }
  get showFieldMapping() {
    return this.featureSettings.fieldMapping;
  }
  get showTranslations() {
    return this.featureSettings.translations;
  }
  get showAssetRepository() {
    return this.featureSettings.assetRepository === true;
  }

  get repositoryModalTitle() {
    return this.repositoryModalKind === "question"
      ? "Select library question"
      : "Select library section";
  }

  get repositoryResultsEmpty() {
    return (
      !this.repositorySearching &&
      (!this.repositoryResults || this.repositoryResults.length === 0)
    );
  }
  ...
  get toolboxItems() {
    ...
    if (
      this.repositoryAssetEditUi &&
      (this.repositoryFocusType || "").toLowerCase() === "section"
    ) {
  ...
  get propSections() {
    const make = (key, defaultOpen) => {
      const isOpen =
        this.expandedSections[key] !== undefined
          ? this.expandedSections[key]
          : defaultOpen;
      return {
        open: isOpen,
        chevron: isOpen ? "utility:chevrondown" : "utility:chevronright"
      };
    };
    return {
  ...
  disconnectedCallback() {
    this._teardownModalFocusTrap();
    this._modalTrapKey = null;
  }

  renderedCallback() {
    const nextKey = this._getActiveModalTrapKey();
    if (nextKey !== this._modalTrapKey) {
      if (this._modalTrapKey) {
        this._teardownModalFocusTrap();
      }
      this._modalTrapKey = nextKey;
      if (nextKey) {
        this._setupModalFocusTrap(nextKey);
      }
    }
  }

  _getActiveModalTrapKey() {
    if (this.showNewFormModal) return "newForm";
    if (this.showRepositoryModal) return "repository";
    if (this.showPromoteConfirm) return "promote";
    if (this.showConfigModal) return "config";
    if (this.showCloneModal) return "clone";
    return null;
  }
  ...
  _setupModalFocusTrap(trapKey) {
    ...
    requestAnimationFrame(() => {
      // eslint-disable-next-line @lwc/lwc/no-async-operation
      requestAnimationFrame(() => this._focusFirstTabbableInModal(trapKey));
    });
  }
  ...
  _focusFirstTabbableInModal(trapKey) {
  ...
  _closeActiveModal() {
    ...
    } else if (this.showRepositoryModal) {
      this.closeRepositoryModal();
    } else if (this.showPromoteConfirm) {
      this.handleCancelPromoteConfirm();
    } else if (this.showConfigModal) {
      this.handleCloseConfigModal();
    } else if (this.showCloneModal) {
      this.handleCancelClone();
    }
  }

  _handleModalKeydownCapture(event) {
    const trapKey = this._modalTrapKey;
    if (!trapKey) {
      return;
    }
    const modalRoot = this.template.querySelector(
  ...
  async refreshFormStructure() {
    ...
    try {
      ...
      if (
        ...
        const elementType =
          focusType === "section" || focusType === "question" ? focusType : "";
        if (elementType) {
          const focused = this.findElementById(
            elementType,
            this.repositoryFocusId
          );
          if (focused) {
        ...
        const refreshed = this.findElementById(
          this.selectedElementType,
  ...
  handleDragOver(event) {
    ...
    event.dataTransfer.dropEffect =
      this.dragType === "reorder" ? "move" : "copy";
  ...
  async handleBuilderDrop(event) {
    ...
    await this.applyUnifiedDropDetail({
      data,
      dropzone,
      parentId,
      position,
  ...
  async handleReorderOrMoveDrop(data, dropzone, parentId, position) {
    ...
    if (itemType === "page") {
      ...
      if (normTarget !== normSource) {
        await this.moveItemToNewParent(
          "page",
          itemId,
          normTarget,
          position + 1
    ...
    const isCrossContainer =
      parentId && sourceParentId && parentId !== sourceParentId;
  ...
  async reorderItem(objectType, itemId, parentId, newPosition) {
    ...
    if (objectType === "category") {
      items = (this.formStructure.categories || []).map((c) => ({
        id: c.Id,
        order: c.C_Order__c
      }));
    } else if (objectType === "page") {
      const catPages = this.getPagesForCategoryKey(
        this._normalizePageCategoryId(parentId)
      );
      items = catPages.map((p) => ({ id: p.Id, order: p.C_Order__c }));
    } else if (objectType === "section") {
      items = (this.formStructure.sectionsByPage?.[parentId] || []).map(
        (s) => ({ id: s.Id, order: s.C_Order__c })
      );
    } else if (objectType === "question") {
      items = (this.formStructure.questionsBySection?.[parentId] || []).map(
        (q) => ({ id: q.Id, order: q.C_Order__c })
      );
    } else if (objectType === "response") {
      items = (this.formStructure.responsesByQuestion?.[parentId] || []).map(
        (r) => ({ id: r.Id, order: r.C_Order__c })
      );
    }
  ...
  async createNewResponse(questionId, position) {
    const responses =
      this.formStructure?.responsesByQuestion?.[questionId] || [];
  ...
  findElementById(type, id) {
    ...
    if (type === "form") {
      return this.formStructure?.form?.Id === id
  ...
  handleTableColumnsChange(event) {
    this.selectedElement = {
      ...this.selectedElement,
      C_Table_Columns__c: event.detail.value
    };
  }
  ...
  async handleSaveDependency() {
    ...
    if (
      !dep.C_Controlling_Response__c ||
      !dep.C_Target_Type__c ||
      !dep.C_Action__c
    ) {
    ...
    const hasTarget =
      dep.C_Target_Category__c ||
      dep.C_Target_Page__c ||
      dep.C_Target_Section__c ||
      dep.C_Target_Question__c;
    if (!hasTarget) {
  ...
  async handleConfirmClone() {
    ...
    try {
      await cloneForm({
        formId: this.selectedFormId,
        newFormName: this.cloneFormName
      });
      this.showCloneModal = false;
      this.dispatchEvent(
        new ShowToastEvent({
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          title: "Error",
          message: error?.body?.message || "Clone failed",
          variant: "error"
        })
      );
    } finally {
  ...
  async loadFieldMappings() {
    try {
      this.fieldMappingTargets = await getFormTargets({
        formId: this.selectedFormId
      });
  ...
  _getQuestionMappingBadge(questionId) {
    ...
    if (
      dirs.has("Bidirectional") ||
      (dirs.has("Inbound") && dirs.has("Outbound"))
    ) {
      return {
        isMapped: true,
        label: "Synced",
        cssClass: "type-badge synced-badge"
      };
    }
    if (dirs.has("Inbound")) {
      return {
        isMapped: true,
        label: "Pre-filled",
        cssClass: "type-badge prefilled-badge"
      };
    }
    return {
      isMapped: true,
      label: "Mapped",
      cssClass: "type-badge mapped-badge"
    };
  }
  ...
  openRepositoryModal(kind, parentId, position) {
    ...
    this.pendingRepositoryPosition =
    ...
    try {
      ...
      if (this.repositoryModalKind === "question") {
        ...
        this.repositoryResults = (rows || []).map((r) => ({
          ...
          title: r.Name,
          subtitle:
            `${r.C_Question_Text__c || ""} · ${r.C_Type__c || ""}`.trim()
        }));
      } else {
  ...
  async handleConfirmPromote() {
    ...
    try {
      if (this.promoteConfirmKind === "question") {
        await promoteQuestionToLibrary({
          formQuestionId: this.selectedElement.Id
        });
        this.showSuccess("Question copied to public repository");
      } else {
        await promoteSectionToLibrary({
          formSectionId: this.selectedElement.Id
        });
  ...
  showError(message, error) {
    ...
    this.dispatchEvent(
      new ShowToastEvent({ title: "Error", message: detail, variant: "error" })
    );
  }
