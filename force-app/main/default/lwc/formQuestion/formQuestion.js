...
export default class FormQuestion extends LightningElement {
  ...
  get wordCounterOverLimit() {
    return (
      this.effectiveMaxWordCount > 0 &&
      this.currentWordCount > this.effectiveMaxWordCount
    );
  }
  ...
  get wordCounterRowClass() {
    ...
    return this.wordCounterOverLimit
      ? `${base} word-counter-row--error`
      : `${base} word-counter-row--muted`;
  }
  ...
  get multiSelectValue() {
    ...
    return String(this.question.value)
      .split(";")
      .filter((v) => v);
  }
  ...
  _applyCharCountdownNativeValidity(
    target,
    value,
    requiredEmptyCustomValidity
  ) {
    if (this.isDisabled) {
  ...
  _applyCharCountdownShortNativeValidity(target, value) {
    this._applyCharCountdownNativeValidity(
      target,
      value,
      REQUIRED_VALUE_MISSING_MESSAGE
    );
  }
  ...
  handlePicklistChange(event) {
    ...
    const selectedResponse = this.question.responses.find(
      (r) => String(r.responseId) === String(selectedValue)
    );
    this.dispatchValueChange(
      selectedValue,
      selectedResponse ? selectedResponse.responseText : ""
    );
  }
  ...
  handleCharCountdownShortBlur(event) {
    this._applyCharCountdownShortNativeValidity(
      event.target,
      event.target.value
    );
    event.target.reportValidity();
  ...
  handleLongTextNativeBlur(event) {
    ...
    if (this.showWordCounter) {
      ...
      this._applyCharCountdownLongNativeValidity(
        event.target,
        event.target.value
      );
    }
  ...
  /**
   * Re-run field validity (regex, required, long-text word limit, cross-field display state)
   * and surface native / base-component messages. Used before submit and conversational "next".
   *
   * @returns {boolean} true if the control passes validation
   */
  @api
  reportInputValidity() {
    if (this.isDisabled) {
    ...
    if (this.isShortText) {
      const shortNative = this.template.querySelector(
        "input.char-countdown-short-native"
      );
      if (shortNative) {
        this._applyCharCountdownShortNativeValidity(
          shortNative,
        ...
        if (typeof shortNative.reportValidity === "function") {
          return shortNative.reportValidity();
        }
        return true;
      }
      const shortLi = this.template.querySelector("lightning-input");
      if (shortLi && typeof shortLi.reportValidity === "function") {
        return shortLi.reportValidity();
      }
      return true;
    }

    if (this.isLongText) {
      if (this.showWordCounter && this.wordCounterOverLimit) {
        this._longTextTouched = true;
        return false;
      }
      this._longTextTouched = true;
      const nativeLong = this.template.querySelector(
        "textarea.long-text-native"
      );
      if (nativeLong) {
        ...
        if (typeof nativeLong.reportValidity === "function") {
          return nativeLong.reportValidity();
        }
        return true;
      }
    ...
    if (this.isNumberQuestion || this.isDateQuestion || this.isFileQuestion) {
      const li = this.template.querySelector("lightning-input");
      if (li && typeof li.reportValidity === "function") {
        return li.reportValidity();
      }
      return true;
    }

    if (this.isPicklist || this.isMultiSelect || this.isCheckboxQuestion) {
      const group =
        this.template.querySelector("lightning-radio-group") ||
        this.template.querySelector("lightning-checkbox-group");
      if (group && typeof group.reportValidity === "function") {
        return group.reportValidity();
      }
      return true;
    }

    if (this.isTableQuestion) {
      const tableEl = this.template.querySelector("c-form-question-table");
      if (tableEl && typeof tableEl.flushPendingCellEdits === "function") {
        tableEl.flushPendingCellEdits();
      }
      return true;
    }
